...
// Some constants
const Vector3f LIGHT_POS(10.0f, 10.0f, 10.0f);
const Vector3f LIGHT_COLOR(0.0f, 0.0f, 0.0f);
...
void freeSystem() {
    delete galaxySystem; galaxySystem = nullptr;
...
void stepSystem()
{
    ...
    int count = 0;
    while (count < 1) {
        timeStepper->takeStep(galaxySystem, h);
        simulated_s += h;
        count += 1;
    }
}
